...
def train_one_epoch(
    ...
    return total_loss / max(n_batches, 1), epoch_secs
...
def parse_args():
    ...
    parser.add_argument("--max_length",    type=int,   default=256) # Maximum token length for the citation context (tokenizer)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        log = {"epoch": epoch, "train_loss": train_loss, "epoch_secs": round(epoch_secs, 2)}
        print(f"Epoch {epoch:3d} | train_loss={train_loss:.4f} | time={epoch_secs:.1f}s", end="")
    ...
    print(f"Total training time: {total_mins:.1f} min ({total_secs:.0f}s)")
