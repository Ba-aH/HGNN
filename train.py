...
def infonce_loss(context_emb: torch.Tensor, paper_emb: torch.Tensor, temperature: float = 0.07) -> torch.Tensor:
    ...
    loss_c2p = F.cross_entropy(logits, labels)
    loss_p2c = F.cross_entropy(logits.T, labels)
    return (loss_c2p + loss_p2c) / 2.0


# ---------------------------------------------------------------------------
# Build frozen candidate index (corpus + external)
# ---------------------------------------------------------------------------
@torch.no_grad()
def build_candidate_index(
    paper_tower:       nn.Module,
    all_paper_feats:   dict,
    candidate_ids:     torch.Tensor,   # Now contains corpus + external
    device:            torch.device,
...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        with torch.amp.autocast('cuda'):
            ...
            paper_emb = paper_tower(batch_paper_feats)
